...
class NetworkManager
  ...
  class _NetworkManager
    ...
    connect: (gameType, gameId, playerId, gameServerAddress, spectatorId=null, spectateToken=null) ->
      ...
      # SP modes use port 6900; MP modes use port 6901.
      port = if GameType.isSinglePlayerGameType(gameType) then 6900 else 6901
      host_test = "duelyst-sp-api.dev.blockdudes.com"
      # Format the WebSocket URL.
      # websocketUrl = "#{protocol}://#{host}:#{port}"
      websocketUrl = "#{protocol}://#{host_test}"
