...
class Session extends EventEmitter
	...
	login: (silent = false) ->
		...
		debug("login")
		Logger.module("WALLET").log "CONNECT WALLET"
		...
		Logger.module("WALLET").log "provider", provider
		try
			dagProvider = provider.getProvider('constellation')
			Logger.module("WALLET").log "dagProvider", dagProvider
		catch error
			Logger.module("WALLET").log "error", error
			return null
		
		Promise.resolve(
			dagProvider.activate()
			Logger.module("WALLET").log "working here as well"
		)
		.then () =>
			return dagProvider.request({method: 'dag_accounts', params: []})
		.then (accounts) =>
			Logger.module("WALLET").log "accounts", accounts
			body.username = accounts[0]
		.then () =>
			return Promise.resolve(
				fetch "#{@url}/session",
					method: 'POST'
	...
	isAuthenticated: (token) ->
		...
		.timeout(15000)
		.then (decodedToken) =>
			Logger.module("isAuthenticated").log "decodedToken", decodedToken
