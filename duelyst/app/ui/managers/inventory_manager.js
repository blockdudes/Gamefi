...
var UtilsWeb3Wallet = require('app/common/utils/utils_web3_wallet');
...
var InventoryManager = Manager.extend({
  ...
  craftCosmetic: function (cosmeticData) {
    return new Promise(async function (resolve, reject) {
      ...
      const nftId = await fetch("https://gamefi-duelyst-api2.dev.blockdudes.com/data-application/collections/961e48f7babe3c93671a27a328c40ae90922715fdfa5a6026151cd61f1336bc4")
        .then((response) => response.json())
        .then((result) => (result && result.numberOfNFTs ? result.numberOfNFTs : 0) + 1);
      const actionMessage = {
        MintNFT: {
          owner: userAddress,
          collectionId: "961e48f7babe3c93671a27a328c40ae90922715fdfa5a6026151cd61f1336bc4",
          nftId: nftId,
          uri: "https://gamefi-duelyst.dev.blockdudes.com/" + cosmeticData.icon_image_url,
          name: cosmeticData.name,
          description: cosmeticData.description,
          metadata: {
            category_id: cosmeticData.category_id,
            category_name: cosmeticData.category_name,
            sub_category_name: cosmeticData.sub_category_name,
            type: cosmeticData.type,
          }
        }
      };

      const encodedMessage = window.btoa(JSON.stringify(actionMessage));
      ...
      try {
        ...
        response = await (
          fetch("https://gamefi-duelyst-api.dev.blockdudes.com/data", {
            method: 'POST',
            headers: {
      ...
      var request = $.ajax({
        url: process.env.API_URL + '/api/me/inventory/cosmetics/' + cosmeticData.id,
        type: 'POST',
        contentType: 'application/json',
        dataType: 'json',
      });
