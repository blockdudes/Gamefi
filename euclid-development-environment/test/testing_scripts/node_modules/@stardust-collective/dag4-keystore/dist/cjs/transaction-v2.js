...
class TransactionV2 {
    constructor({ fromAddress, toAddress, amount, fee, lastTxRef, salt }) {
        ...
        if (salt === undefined) {
            salt = new bignumber_js_1.BigNumber(MIN_SALT + parseInt((0, crypto_1.randomBytes)(6).toString('hex'), 16));
        }
    ...
    static toHexString(val) {
        val = new bignumber_js_1.BigNumber(val);
        let bInt;
        if (val < new bignumber_js_1.BigNumber(0)) {
            bInt = (1 << 64) + val;
        }
        else {
            bInt = val;
    ...
    getPostTransaction() {
        return {
            value: Object.assign(Object.assign({}, this.tx.value), { salt: this.tx.value.salt.toString().replace('n', '') }),
